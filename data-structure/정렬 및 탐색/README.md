...
힙 생성 알고리즘은, 전체 트리에 대해서 heapify를 반복적으로 수행하면 전체 노드가 다 힙 구조가 된다. 한번 heapify  
가 수행되는 시간은 트리의 높이와 같기 때문에 **log<sub>2</sub>N**이라고 할 수 있다. 모든 정점 N만큼 곱해주면 ,  
**N×logN** 으로 힙 구조를 만들 수 있다. 실제로 데이터가 5개일 때는, 5를 2로 나눈 2번 만큼만 heapify를 수행하면 된다.  
특정한 숫자의 1/2만큼만 수행하더라도 힙 구조를 만들수 있다. 따라서 1/2N×logN이라고 할 수 있는데, 1/2N이 logN보다 더 크다는  
점에서 그냥 N으로 바꿀 수 있다. **다시 말해서 전체 트리를 힙으로 만드는 시간 복잡도는 O(N)이 된다.**
